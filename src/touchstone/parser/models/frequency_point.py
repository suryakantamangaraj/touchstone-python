...
class FrequencyPoint:
    ...
    def __post_init__(self):
        ...
        if self._s_parameters.ndim != 2 or self._s_parameters.shape[0] != self._s_parameters.shape[1]:
            raise ValueError(f"S-parameters must be a square matrix. Got shape {self._s_parameters.shape}")
    ...
    def __getitem__(self, indices: tuple[int, int]) -> NetworkParameter:
        ...
        if not (0 <= row < self.number_of_ports and 0 <= col < self.number_of_ports):
            raise IndexError(f"Index [{row}, {col}] is out of range for {self.number_of_ports}-port data.")
