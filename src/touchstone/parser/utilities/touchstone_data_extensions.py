...
def get_magnitude(data: TouchstoneData, to_port: int, from_port: int, db: bool = True) -> np.ndarray:
    """Get magnitude of S-parameters (defaults to dB)."""
    s = data.get_s(to_port, from_port)
    mag = np.abs(s)
    return mag_to_db(mag) if db else mag

def get_phase(data: TouchstoneData, to_port: int, from_port: int, deg: bool = True) -> np.ndarray:
