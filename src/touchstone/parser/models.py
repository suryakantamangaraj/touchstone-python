...
class TouchstoneData:
    ...
    def __post_init__(self):
        object.__setattr__(self, 'n_ports', self.s_parameters.shape[1])
        object.__setattr__(self, 'n_freq', len(self.frequency))
        if self.s_parameters.shape[0] != self.n_freq:
            raise ValueError(f"Frequency length ({self.n_freq}) must match S-parameter length ({self.s_parameters.shape[0]})")
    ...
    def __repr__(self) -> str:
        return (f"TouchstoneData(n_ports={self.n_ports}, n_freq={self.n_freq}, "
